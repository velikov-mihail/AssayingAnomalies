...
driverLocationDir = (dir(fullfile(Params.directory,'**', filesep, 'postgresql*.jar')));
if isempty(driverLocationDir)
    ...
    if path==0
        error('ERROR: Wrong PostgreSQL JDBC driver location.');
    else
        driverLocation = [path, file];
    end    
else
    driverLocation = [driverLocationDir.folder, filesep, driverLocationDir.name];
end
...
if status == 1
    ...
    error('\nERROR: Connection to WRDS unsuccessful. Check JAVA driver installation.\n');
end
